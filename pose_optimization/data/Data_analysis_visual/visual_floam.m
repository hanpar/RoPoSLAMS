...
gtkitti20110930drive0018 = importfile3("../floam_gt_kitti_2011_09_30_drive_notsync.txt", [2, Inf]);
gtkitti_t = importfile4("../floam_gt_kitti_2011_09_30_drive_notsync.txt", [2, Inf]);
optpose = importfile1("../../optimized_poses_with_imu_kitti_0018_notsync.txt", [2, Inf]);
optpose_floam = importfile1("../../optimized_poses_without_imu_kitti_0018_notsync.txt", [2, Inf]);
...
meanerror_floamx = mean(abs(error_floamx),'omitnan')

meanerror_fusiony = mean(abs(error_fusiony),'omitnan')
meanerror_floamy = mean(abs(error_floamy),'omitnan')

stdfx = std(error_fusionx,'omitnan')
stdfy = std(error_fusiony,'omitnan')

stdfloamx = std(error_floamx,'omitnan')
stdfloamy = std(error_floamy,'omitnan')
